...
<head>
    ...
    <title>{{ $currentArticle->title ?? 'Cẩm Nang Cho Mẹ Và Bé' }}</title>
...
<body class="bg-gray-50 text-gray-800">
    ...
    <div class="max-w-7xl mx-auto flex min-h-screen">
        ...
        <!-- SIDEBAR BÊN TRÁI: MỤC LỤC SÁCH -->
        <aside class="w-80 bg-white border-r border-gray-200 p-6 overflow-y-auto h-screen sticky top-0">
            <a href="/" class="font-bold text-xl text-[#FF2A54] block mb-6">MommyKids Handbook</a>
        ...
        <!-- NỘI DUNG BÀI VIẾT CHÍNH BÊN PHẢI -->
        <main class="flex-1 p-10 bg-white">
            @if($currentArticle)
                <article class="max-w-3xl mx-auto">
                    ...
                    <h1 class="text-3xl font-extrabold text-gray-900 mt-3 mb-6">
                    ...
                    <div class="mt-12 pt-6 border-t border-gray-200 flex items-center justify-between">
                        @if(isset($prevArticle) && $prevArticle)
                            <a href="{{ route('handbook.show', $prevArticle->slug) }}" class="text-sm font-semibold text-gray-600 hover:text-[#FF2A54]">
                        ...
                        @if(isset($nextArticle) && $nextArticle)
                            <a href="{{ route('handbook.show', $nextArticle->slug) }}" class="text-sm font-semibold text-gray-600 hover:text-[#FF2A54]">
