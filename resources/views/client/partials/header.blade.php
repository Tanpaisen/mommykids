<header class="sticky top-0 z-50 bg-surface/95 backdrop-blur border-b border-coral-light">
    {{-- Thanh thông báo chạy chữ trên cùng (Top Announcement) --}}
    @if(!empty($globalSetting->top_announcement))
        <div class="bg-coral text-white text-xs py-1 px-4 text-center font-medium">
            {{ $globalSetting->top_announcement }}
        </div>
    @endif
    ...
    <div class="max-w-[1280px] mx-auto px-4 lg:px-6 py-3 flex items-center gap-3 lg:gap-6">
        ...
        {{-- Logo & Tên Website (Dynamic từ Admin) --}}
        <a href="{{ url('/') }}" class="flex items-center gap-2 shrink-0">
            @if(!empty($globalSetting->logo))
                <img src="{{ asset('storage/' . $globalSetting->logo) }}" alt="{{ $globalSetting->site_name ?? 'MommyKids' }}" class="h-10 w-auto object-contain">
            @else
                ...
                <span class="hidden sm:block font-display font-extrabold text-xl text-ink">
                    {{ $globalSetting->site_name ?? 'MommyKids' }}
                </span>
            @endif
        </a>

        {{-- Location picker (desktop - Dynamic từ Admin) --}}
        <button class="hidden lg:flex items-center gap-1 text-sm text-ink-soft px-3 py-2 rounded-pill border border-coral-light hover:border-coral shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-coral" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a2 2 0 11-2.828 0l-4.243-4.243a8 8 0 1111.314 0z" />
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            Giao tới: <span class="font-semibold text-ink">{{ $globalSetting->header_address ?? ($currentCity ?? 'Hà Nội') }}</span>
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
            </svg>
        </button>

        {{-- Search bar (Dynamic placeholder từ Admin) --}}
        <form action="{{ route('search') ?? '#' }}" method="GET" class="flex-1 flex items-center">
            <div class="relative w-full">
                <input type="text" name="q"
                       placeholder="{{ $globalSetting->search_placeholder ?? 'Ba mẹ cần tìm gì cho bé hôm nay?' }}"
                       class="w-full h-11 pl-4 pr-11 rounded-pill border-2 border-coral-light focus:border-coral outline-none text-sm placeholder:text-ink-soft/70">
                <button type="submit" class="absolute right-1.5 top-1.5 w-8 h-8 rounded-full bg-coral text-white flex items-center justify-center hover:bg-coral-dark">
    ...
    <div class="hidden lg:block bg-coral-light/60">
        <div class="max-w-[1280px] mx-auto px-6 py-1.5 flex items-center gap-6 text-xs font-medium text-ink-soft">
            ...
            <a href="tel:{{ $globalSetting->hotline }}" class="hover:text-coral">Hotline: {{ $globalSetting->hotline ?? '1800 6886' }}</a>
