...
<body class="bg-cream text-ink font-body">
    ...
    <a href="{{ route('cart.index') ?? '#' }}"
       class="lg:hidden fixed bottom-5 right-5 z-40 w-14 h-14 rounded-full bg-coral shadow-pop
        ...
        <span class="mk-cart-count absolute -top-1 -right-1 bg-mint text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center">
            {{ $cartCount ?? 0 }}
        </span>
    </a>
    ...
    {{-- Toast --}}
    <div id="mk-toast"
         class="hidden fixed top-20 left-1/2 -translate-x-1/2 z-[60] bg-ink text-white text-sm px-4 py-2 rounded-pill shadow-pop"></div>
